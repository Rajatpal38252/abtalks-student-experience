...
function dayMission(day) {
  if (day === 1) return { title: 'Build a Personal Portfolio', description: 'Create a polished personal portfolio that introduces your work, skills, and the projects you want recruiters to see.', focus: 'Portfolio foundation', time: '2 Hours' };
  const missions = [['Build a responsive component', 'Turn a small interface idea into a responsive component with thoughtful states.', 'UI systems'], ['Ship a useful interaction', 'Add a clear user interaction and document the decision behind it.', 'Product thinking'], ['Polish the experience', 'Improve visual hierarchy, keyboard support, and feedback states.', 'Craft']];
  const [title, description, focus] = missions[(day - 2) % missions.length];
  return { title, description, focus, time: '45–60 min' };
}
